...
export default function SpeechTextarea({
  ...
  const handleSpeechTranscript = (transcript: string) => {
    ...
    if (textarea) {
      ...
      if (maxLength && newValue.length > maxLength) {
        showError(
          language === 'es' 
            ? `El texto excede el límite de ${maxLength} caracteres`
            : `Text exceeds the limit of ${maxLength} characters`
        )
        return
      }
  ...
  const handleSpeechError = (error: string) => {
    ...
    switch (error) {
      case 'network-error':
        errorMessage = t.speech.networkError
        break
      case 'microphone-access-denied':
        errorMessage = t.speech.microphoneAccess
        break
      case 'no-speech-detected':
        errorMessage = t.speech.noSpeechDetected
        break
      case 'audio-not-supported':
        errorMessage = t.speech.audioNotSupported
        break
      default:
        if (error.startsWith('speech-error-')) {
          errorMessage = t.speech.transcriptionError
        }
        break
    }
