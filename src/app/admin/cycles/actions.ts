...
export async function createCycle(formData: FormData) {
  try {
    ...
    // Note: For now, we skip audit logging for cycles since AuditLog is designed for evaluations
    // This could be extended with a more generic audit system in the future

    revalidatePath('/admin/cycles')
    revalidatePath('/dashboard') // Refresh dashboard cycle selector
...
export async function updateCycleStatus(cycleId: string, formData: FormData) {
  try {
    ...
    // If closing a cycle, ensure there are no other active cycles being created simultaneously
    if (status === 'active') {
      const activeCycle = await prisma.performanceCycle.findFirst({
        where: {
          companyId: currentUser.companyId,
    ...
    // Note: Audit logging skipped - AuditLog is designed for evaluations
