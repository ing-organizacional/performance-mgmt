...
export default function CompanyItemsPage() {
  ...
  const [companyItems, setCompanyItems] = useState<CompanyEvaluationItem[]>([])
  ...
  const fetchCompanyItems = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/evaluation-items/all?includeInactive=true')
      if (response.ok) {
        const data = await response.json()
        // Filter only company-level items
        const companyLevelItems = data.items.filter((item: CompanyEvaluationItem) => 
          item.level === 'company'
        )
        setCompanyItems(companyLevelItems)
      }
    } catch (error) {
  ...
  const handleSaveNew = async () => {
    ...
    try {
      ...
      if (response.ok) {
        await fetchCompanyItems() // Refresh data
        setEditingItem(null)
